...
export class ShopComponent {
  ...
  this.productService.getCount(this.activeIndex,0,50000).subscribe(
    {
      next:(res:any)=>{
...
    })
  ...
  filter(){
   ...
   this.productService.getCount(this.activeIndex,0,50000).subscribe(
    ...
    this.productService.getPage(this.page,this.activeIndex,0,50000).subscribe(
      {
        next:(res:any)=>{
          this.productService.products=res;
      }
...
  }
  ...
  changeBreadcrumbBackground(imageUrl: string) {
    const breadcrumbSection = document.querySelector('.breadcrumb-section') as HTMLElement;
    breadcrumbSection.style.backgroundImage = `url(${imageUrl})`;
  }
